...
namespace ControleDeVinil.WebApi.Controllers
{
    ...
    public class ProdutorController : ControllerBase
    {
        ...
        [HttpPost]
        public IActionResult Criar(Produtor novoProdutor)
        {
        ...
        [HttpPut("{id}")]
        public IActionResult Atualizar(int id, Produtor produtorAtualizado)
        {
            if (produtorAtualizado == null ||produtorAtualizado.Id != id)
            {
                return BadRequest("Dados inválidos");
            }
            ...
            ProdutorDAO.Atualizar(produtorAtualizado);
